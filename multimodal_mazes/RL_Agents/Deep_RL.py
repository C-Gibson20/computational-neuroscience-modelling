...
class GridPlotterContinuous:
    ...
    def plot_episode(self, trial_data, ax=None):
        ...
        ax.add_patch(patches.Circle((agent_path[0][1], agent_path[0][0]), radius=0.5, color=colors[0], zorder=2))
...
class DDPGAgent:
    ...
    def rewards(self, location, next_location, prey_location, action, next_location_valid):
        if np.linalg.norm(prey_location - location) < self.capture_radius:
            ...
            acceleration = 0 if self.last_action is None else self.calculate_acceleration(self.last_action, action)
            # return 200 * (current_distance - next_distance + 1/current_distance) - 50 * acceleration
            return ((10 * (current_distance - next_distance)) - 1  - 2 * acceleration, False)
        else:
            return (-20, False)
    ...
    def calculate_acceleration(self, last_action, action):
        v1 = last_action[0]
        theta1 = last_action[1]

        v2 = action[0]
        theta2 = action[1]
        
        ax = (v2 * np.cos(theta2)) - (v1 * np.cos(theta1))
        ay = (v2 * np.sin(theta2)) - (v1 * np.sin(theta1))
    ...
    def act(self, env, prey_locations, prey_speeds, prey_visible=True, noise_scale=0.0, training=False):
        ...
        state_vector = self.state_vector(self.location, noise_scale)
        ...
        if training:
            ...
            reward, done = self.rewards(self.location, next_location, nearest_prey, action, next_location_valid)
            next_state_vector = self.state_vector(next_location, noise_scale) if next_location_valid else state_vector
    ...
    def produce_plots(self, plot_training_lengths, plot_first_5_last_5, plot_percentage_captured, 
        ...
        if plot_first_5_last_5:
            ...
            for i in range(5):
                ...
                ax_first = axs[0, i]
                self.plotter.plot_env(trial_first['env'][0], ax=ax_first)
                self.plotter.plot_prey(trial_first['env'][0], trial_first['prey_locations'][0], ax=ax_first)
                self.plotter.plot_episode(trial_first, ax=ax_first)
                
                # Last 5 trials
                trial_last = trials[len(trials)-(i+1)]
                ax_last = axs[1, 4 - i]
                self.plotter.plot_env(trial_last['env'][0], ax=ax_last)
                self.plotter.plot_prey(trial_last['env'][0], trial_last['prey_locations'][0], ax=ax_last)
